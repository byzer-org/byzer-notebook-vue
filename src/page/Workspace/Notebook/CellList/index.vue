...
export default {
    ...
    selectCell: {
      handler (newVal, oldValue) {
        ...
        if (id) {
          ...
          if (
            editType === 'Markdown' &&
            (oldEditType === 'MLSQL' || oldEditType === 'Python')
          ) {
  ...
  methods: {
    ...
    doReplace (param) {
      ...
      this.newCellList = this.newCellList.map((item, index) => {
        if (item.content && item.id === cellId) {
          ...
          if (item.editType === 'MLSQL' || item.editType === 'Python') {
    ...
    changeMode (mode) {
      ...
      if (
        mode === 'command' &&
        (this.selectCell.editType === 'MLSQL' ||
    ...
    async handleAddCell (data, cell) {
      ...
      try {
        ...
        this.loadingSave = false
        this.newCellList = this.dataProcess(res.data.cell_list)
        this.selectCell = Object.assign(newCell.data, { editType: 'MLSQL' })
        this.changeMode('edit')
