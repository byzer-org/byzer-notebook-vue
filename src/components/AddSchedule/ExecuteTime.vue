...
export default {
  ...
  methods: {
    ...
    expressionChange () {
      ...
      let ruleArr = this.scheduleForm.cron.split(' ')
      // 用于记录进入循环的次数
      let nums = 0
      // 用于暂时存符号时间规则结果的数组
      let resultArr = []
      ...
      if (resultArr.length === 0) {
        executionTimeResult = [this.$t('crontab.noResult')]
      } else {
        executionTimeResult = resultArr
        if (resultArr.length !== 5) {
          executionTimeResult.push(
            this.$t('crontab.msg', { len: resultArr.length })
          )
        }
      }
      this.executionTimeResult = executionTimeResult.join('\n')
    },
    // 用于计算某位数字在数组中的索引
    getIndex (arr, value) {
    ...
    getYearArr (rule, year) {
      this.dateArr[5] = this.getOrderArr(
        year,
        year + 100 > 2099 ? 2099 : year + 100
      )
      if (rule !== undefined) {
        if (rule.indexOf('-') >= 0) {
